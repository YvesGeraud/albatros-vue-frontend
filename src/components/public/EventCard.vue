...
<template>
  <RouterLink
    ...
    class="abt-surface d-block p-3 h-100 text-decoration-none"
  >
    <div class="d-flex justify-content-between align-items-start mb-2">
      <span class="abt-mono small abt-text-muted">{{ formattedDate(event.event_date) }}</span>
      <LiveBadge v-if="event.is_live" />
      <span v-else-if="event.status === 'upcoming'" class="abt-mono small abt-text-cyan">PRÓXIMO</span>
    </div>
